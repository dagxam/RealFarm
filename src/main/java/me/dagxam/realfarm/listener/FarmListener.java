...
import me.dagxam.realfarm.farm.GreenhouseDetector;
...
public final class FarmListener implements Listener {
    ...
    private final GreenhouseDetector greenhouseDetector = new GreenhouseDetector();
    ...
    public void onCropOrSaplingPlace(BlockPlaceEvent event) {
        ...
        if (treeGrowthManager.isManaged(block)) {
            treeGrowthManager.register(block);
            return;
        }
        if (block.getBlockData() instanceof Ageable && cropGrowthManager.isManaged(block) && validator.findFarm(block) != null) {
    ...
    public void onMoistureChange(MoistureChangeEvent event) {
        ...
        if (farm == null) return;
        stateManager.refresh(farm);
        if (shouldStayWet(farm)) {
            event.setCancelled(true);
            setMoisture(event.getBlock(), true);
        } else if (isWinter(farm)) {
            event.setCancelled(true);
    ...
    private void showTargetInfo(Player player) {
        ...
        String type = null;
        if (stateManager.isFarmCauldron(target)) {
            farm = findFarmForService(target);
            type = "котёл";
        }
        if (stateManager.isFarmComposter(target)) {
            farm = findFarmForService(target);
            type = "компостер";
        }
        ...
        boolean greenhouse = greenhouseDetector.isGreenhouse(farm);
        ...
        String greenhouseText = greenhouse ? "§7| §bПарник: §aДА" : "§7| §bПарник: §cНЕТ";
        String text = "§6Участок №" + plot + " §7| §fПашня: §e" + farm.farmlandCount() + " §7| §fПосажено: §e" + planted + " §7| " + own + greenhouseText + " §7| " + status;
    ...
    private void updateFarmSoil(FarmStructure farm) {
        ...
        for (FarmStructure.BlockPosition position : farm.farmland()) {
            setMoisture(farm.world().getBlockAt(position.x(), position.y(), position.z()), wet);
        }
    }

    private boolean shouldStayWet(FarmStructure farm) {
        // Полностью закрытый стеклянный парник защищает пашню от всех сезонных правил.
        if (greenhouseDetector.isGreenhouse(farm)) return true;
    ...
    private boolean isWinter(FarmStructure farm) {
        return seasons.getSeason(farm.world()) == SeasonIntegration.Season.WINTER;
    }
    ...
    private void notifyInactive(FarmStructure farm) {
        String reason = !farm.hasCauldron() ? "§cНет котла фермы рядом с пашней."
                : !farm.hasComposter() ? "§cНет компостера фермы рядом с пашней."
                : !farm.isWatered() ? "§cНет воды в котле фермы."
                : "§cНет костной муки в компостере фермы.";
